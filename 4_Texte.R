...
#Parametrage
PallierConfidence <- 30   #de 0 (garde tout) à 100 
...
for (i in 1:nb_lignes)
{
  ...
  if (image != "") 
  {
    ...
    image_traitee <- magick::image_contrast(image_traitee, sharpen = 1) #augm. contraste
    ...
    texte_filtre <- texte_brut %>% filter(confidence > PallierConfidence) 
    ...
    dbimages$texte_V5_idem_conf30_N3[i] <- as.character(le_texte)
    
    # #on supprime tous les espaces temporairement pour ne garder que les lettres. 
    # TexteSansEspaces <- gsub(pattern = "\\s", replacement = "", x = le_texte)
    # #Si aucun caractere = pas de texte (car des espaces peuvent trainer)
    # if (TexteSansEspaces == "") {dbimages$texteOuiNon[i] <- 0 } else {dbimages$texteOuiNon[i] <- 1 }
